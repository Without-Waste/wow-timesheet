...
const startBtn = $("startShift");
  ...
  exportCSV.addEventListener("click", ()=>{
    ...
    const ended = [...state.endedShifts].sort((a,b)=> (a.startMs||0)-(b.startMs||0));
    for(const s of ended){
      const startDate = localISODateFromMs(s.startMs);
      const endDate = localISODateFromMs(s.endMs);
      const startTime = format12hFromMsLocal(s.startMs);
      const endTime = format12hFromMsLocal(s.endMs);

      const breakMin = (s.breakMinutes || 0);
      const totalMin = Math.max(0, Math.round((s.endMs - s.startMs)/60000));
      const workedMin = Math.max(0, totalMin - breakMin);
      const workedHours = (workedMin/60).toFixed(2); // e.g. 7.75
