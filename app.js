...
(() => {
  ...
  function applyEndTimeFromInput(shift){
    ...
    if(!/^\d{2}:\d{2}$/.test(t)) return true; // ignore if blank/invalid
  ...
  function openSignatureFor(shift){
    ...
    // Prefill end time nearest quarter hour based on current end (set to now) to avoid iOS weirdness with uninitialised time inputs
    try{
      ...
      sigEndTime.value = nowTimeHHMMRounded();
    }
