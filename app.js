...
(() => {
  ...
  function normalizeShiftStructure(){
    if(!state || !Array.isArray(state.shifts)) return;
    for(const s of state.shifts){
      if(typeof s.notes === "undefined") s.notes = "";
      if(typeof s.shiftId === "undefined") s.shiftId = "";
    }
  }

function saveState(){
    normalizeShiftStructure();
  ...
  function format12hFromHHMM(hhmm){
  ...
  function roundDownToQuarter(ms){
    return Math.floor(ms / (15*60000)) * (15*60000);
  }
  function roundUpToQuarter(ms){
    return Math.ceil(ms / (15*60000)) * (15*60000);
  }
  ...
  function buildQuarterHourOptionsHtml(minMs, maxMs){
    const now = Date.now();
    const min = Number.isFinite(minMs) ? minMs : (now - 12*60*60000);
    const max = Number.isFinite(maxMs) ? maxMs : (now + 60*60000);

    let start = roundDownToQuarter(min);
    let end = roundUpToQuarter(max);

    // Safety: cap list size (max 24h window)
    const cap = start + 24*60*60000;
    if(end > cap) end = cap;
    ...
    for(let t = start; t <= end; t += 15*60000){
      ...
      out += `<option value="${hhmm}">${format12hFromHHMM(hhmm)}</option>`;
    }
  ...
  function refreshNameDatalists(){
    if(!firstNameList || !lastNameList) return;
    const firsts = new Set();
    const lasts = new Set();
    for(const s of (state.shifts || [])){
      const parts = splitName(s.name || "");
      if(parts.first) firsts.add(parts.first);
      if(parts.last) lasts.add(parts.last);
    }
    firstNameList.innerHTML = Array.from(firsts).sort((a,b)=>a.localeCompare(b)).map(v=>`<option value="${escapeHtml(v)}">`).join("");
    lastNameList.innerHTML  = Array.from(lasts).sort((a,b)=>a.localeCompare(b)).map(v=>`<option value="${escapeHtml(v)}">`).join("");
  }
...
function initialsFromName(name){
    ...
    if(parts.length >= 2){
      const a = clean(parts[0]).slice(0,1);
      const b = clean(parts[parts.length-1]).slice(0,1);
      return (a+b) || "XX";
    }
    const one = clean(parts[0] || "");
    if(one.length >= 2) return one.slice(0,2);
    if(one.length === 1) return one + "X";
    return "XX";
  }

  function makeShiftId(dateISO, startMs, name){
    // DDMM-HHMM-Initials (local time)
    ...
    return `${dd}${mm}-${hh}${mi}-${init}`;
  }

  function ensureUniqueShiftId(baseId){
    const used = new Set(state.shifts.map(s => s.shiftId).filter(Boolean));
    ...
    // last resort
    return `${baseId}-${Math.floor(Math.random()*90+10)}`;
  }
  ...
  function paidBreakMinutes(shift){
    return (shift.breaks || []).reduce((sum,b)=>{
      const t = String(b.type || "").toLowerCase().trim();
      const mins = Number(b.minutes)||0;
      return sum + (t === "10 min" ? mins : 0);
    }, 0);
  }

  function unpaidBreakMinutes(shift){
    return (shift.breaks || []).reduce((sum,b)=>{
      const t = String(b.type || "").toLowerCase().trim();
      const mins = Number(b.minutes)||0;
      return sum + ((t === "30 min" || t === "custom" || t.startsWith("custom")) ? mins : 0);
    }, 0);
  }
  ...
  function workedMinutes(shift){
    if(!shift.end) return null;
    return Math.max(0, totalMinutes(shift) - unpaidBreakMinutes(shift));
  }
  ...
  const firstNameList = $("firstNameList");
  const lastNameList  = $("lastNameList");
  const dateInput  = $("shiftDate");
  const timeInput  = $("shiftStart");
  refreshNameDatalists();
  
  function applyDefaults(force=false){
    if(force || !dateInput.value) dateInput.value = todayISO();
    if(force || !timeInput.value) timeInput.value = nearestQuarter();
  }
  applyDefaults(true);
  window.addEventListener("pageshow", ()=>applyDefaults(false));
  document.addEventListener("visibilitychange", ()=>{
    if(document.visibilityState === "visible") applyDefaults(false);
  });
const startBtn = $("startShift");
  ...
  startBtn.addEventListener("click", ()=>{
    ...
    if(!event || !name || !(lastName?.value || "").trim() || !date){
      alert("Please enter event name, first name, last name, and date.");
      return;
    }
    ...
    /** @type {Shift} */
    const shift = {id: uuid(), shiftId: null, event, name, date, start, end: null, breaks: [], notes: "", signatureDataUrl: null};
    shift.shiftId = ensureUniqueShiftId(makeShiftId(date, start, name));
...
function renderActive(){
    ...
    for(const shift of active){
      ...
      const elapsedMin = Math.max(0, Math.round((Date.now() - shift.start)/60000));
      ...
      card.innerHTML = `
        <div class="top">
          <div>
            ...
            <div class="small">Start: ${niceTime(shift.start)} · Live elapsed: ${formatDuration(elapsedMin)}</div>
          </div>
  ...
  if(sigEndTimeSelect && (!sigEndTimeSelect.options || !sigEndTimeSelect.options.length)){
    sigEndTimeSelect.innerHTML = buildQuarterHourOptionsHtml(Date.now() - 12*60*60000, Date.now() + 60*60000);
  }
  ...
  function openSignatureFor(shift){
    ...
    if(sigNotes) sigNotes.value = (typeof shift.notes === "string" ? shift.notes : "");
  ...
  sigSave.addEventListener("click", ()=>{
    ...
    sigTargetShift.notes = (sigNotes?.value || "").trim();
    saveState();
    sigTargetShift = null;
    closeSigDialog();
    renderActive();
  ...
  detailSave.addEventListener("click", ()=>{
    ...
    const notesVal = (detailBody.querySelector("[data-edit-notes]")?.value || "").trim();
    s.notes = notesVal;
  ...
  function openDetail(id){
    ...
    detailBody.innerHTML = `
      ...
      <label class="field" style="margin-top:12px">
        ...
        <textarea data-edit-notes rows="3" placeholder="Anything payroll should know">${escapeHtml(typeof s.notes === "string" ? s.notes : "")}</textarea>
      </label>
  ...
  exportCSV.addEventListener("click", ()=>{
    ...
    for(const s of ended){
      ...
      const breaks = (s.breaks || []).map(b => {
        const t = (b.type || "").trim();
        const mins = Number(b.minutes)||0;
        if(t.toLowerCase() === "custom") return `Custom ${mins} min`;
        if(t) return t; // "10 min" / "30 min"
        return `${mins} min`;
      }).join("; ");

      const paid = paidBreakMinutes(s);
      const unpaid = unpaidBreakMinutes(s);
...
function downloadText(filename, text){
  ...
  renderActive();
  renderHistory();
  setInterval(()=>{ try{ renderActive(); }catch(e){} }, 30000);
})();
