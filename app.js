...
(() => {
  ...
  function nowTimeHHMMRounded(){
    const d = new Date();
    const mins = d.getHours()*60 + d.getMinutes();
    let q = Math.round(mins/15)*15; // nearest quarter hour
    if(q >= 24*60) q = 0; // avoid 24:00 (invalid for <input type="time">)
    const hh = Math.floor(q/60) % 24;
    const mm = q % 60;
    return `${pad2(hh)}:${pad2(mm)}`;
  }

  function niceTime(ms){ return format12hFromMs(ms); }:${pad2(d.getMinutes())}`;
  }
