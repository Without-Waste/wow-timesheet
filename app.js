...
(() => {
  ...
  function ensureEndTimeOptions(){
    if(!sigEndTimeSelect) return;
    if(sigEndTimeSelect.options && sigEndTimeSelect.options.length) return;
    const opts = buildQuarterHourOptions();
    sigEndTimeSelect.innerHTML =
      opts.map(t=>`<option value="${t}">${t}</option>`).join("") +
      `<option value="CUSTOM">Custom…</option>`;
  }

// ---- signature modal ----
  // end time dropdown (more reliable than <input type="time"> on iPad)
  ensureEndTimeOptions();
  ...
  function applyEndTimeFromInput(shift){
    // Use select by default; allow Custom HH:MM
    let t = (sigEndTimeSelect?.value || "").trim();
    if(t === "CUSTOM"){
      t = (sigEndTimeCustom?.value || "").trim();
    }
    if(!/^\d{2}:\d{2}$/.test(t)){
      alert("Please choose a valid end time (HH:MM).");
      return false;
    }
    const ok = setShiftTimeFromHHMM(shift, "end", t);
    if(!ok) return false;
  ...
  function openSignatureFor(shift){
    ...
    // Prefill end time to nearest quarter hour based on current end (set to now)
  ...
  sigCancel.addEventListener("click", ()=>{
    // Back: do NOT end the shift. Revert end time and keep it active.
    if(sigTargetShift){
      sigTargetShift.end = null;
      saveState();
    }
    sigTargetShift = null;
    closeSigDialog();
