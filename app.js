...
(() => {
  ...
  const timeSelect  = $("shiftStartSelect");
  const timeCustom  = $("shiftStartCustom");
  const startBtn = $("startShift");
  const clearBtn = $("clearInputs");

  dateInput.valueAsDate = new Date();
  if(timeSelect && (!timeSelect.options || !timeSelect.options.length)){
    timeSelect.innerHTML = buildQuarterHourOptionsHtml();
  }
  timeSelect.value = nowTimeHHMMRounded();

  startBtn.addEventListener("click", ()=>{
    const event = (eventInput.value || "").trim();
    const name = (nameInput.value || "").trim();
    const date = (dateInput.value || "").trim();
    let time = (timeSelect.value || "").trim();
    if(time === "CUSTOM"){
      time = parse12hToHHMM(timeCustom.value);
      if(!time){
        alert("Custom start time must look like 1:05 pm (or 13:05).");
        return;
      }
    }
