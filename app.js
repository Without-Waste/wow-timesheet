...
(() => {
  ...
  function applyDefaults(){
    const di = document.getElementById("shiftDate");
    const ti = document.getElementById("shiftStart");
    if(!di || !ti) return;
    di.value = todayISO();
    ti.value = nearestQuarter();
  }
  function scheduleDefaults(){
    // Run once now, then a few retries to beat iOS paint quirks
    applyDefaults();
    let tries = 0;
    const h = setInterval(()=>{
      tries++;
      ...
      const di = document.getElementById("shiftDate");
      const ti = document.getElementById("shiftStart");
      if((di && di.value && ti && ti.value) || tries >= 6) clearInterval(h);
    }, 300);
  }
  window.addEventListener("pageshow", scheduleDefaults);
  document.addEventListener("visibilitychange", ()=>{
    if(document.visibilityState === "visible") scheduleDefaults();
  });
  // DOM ready
  if(document.readyState === "loading"){
    document.addEventListener("DOMContentLoaded", scheduleDefaults);
  }else{
    scheduleDefaults();
  }
  // --- End forced defaults ---
  ...
  function parse12hToHHMM(text){
    ...
    if(!m) return null;
    let h = Number(m[1]);
    let mm = m[2] === undefined ? 0 : Number(m[2]);
    const ap = m[3];
  ...
  function buildQuarterHourOptionsHtml(){
    ...
    for(let h=0; h<24; h++){
      for(let m=0; m<60; m+=15){
        ...
        out += `<option value="${t}">${format12hFromHHMM(t)}</option>`;
      }
    }
  ...
  function niceTime(ms){ return format12hFromMs(ms); }
...
const startBtn = $("startShift");
  ...
  timeInput.value = nowTimeHHMMRounded();
  timeInput.setAttribute("value", timeInput.value);

  // iPad Safari quirk: if the time input's value is empty/invalid, the picker defaults to "now".
  // Reinforce the rounded default on focus and on pageshow (bfcache restores).
  ...
  // One extra tick to beat iOS rendering quirks
  setTimeout(()=>{
    if(!dateInput.value){
      dateInput.value = todayISO();
      dateInput.setAttribute("value", dateInput.value);
  ...
  function applyEndTimeFromInput(shift){
    ...
    if(t === "CUSTOM"){
      t = parse12hToHHMM(sigEndTimeCustom?.value || "");
      if(!t){
        alert("Custom end time must look like 1:05 pm (or 13:05).");
        return false;
      }
    }
    if(!/^\d{2}:\d{2}$/.test(t)){
  ...
  detailSave.addEventListener("click", ()=>{
    ...
    const startHHMM = (/^\d{2}:\d{2}$/.test(startVal)) ? startVal : (parse12hToHHMM(startVal) || null);
    const endHHMM = (/^\d{2}:\d{2}$/.test(endVal)) ? endVal : (parse12hToHHMM(endVal) || null);

    if(startHHMM) setShiftTimeFromHHMM(s, "start", startHHMM);
    if(endHHMM) setShiftTimeFromHHMM(s, "end", endHHMM);
    ...
    detailBody.innerHTML = `
      ...
      <div class="grid" style="margin-top:12px">
        <label class="field">
          ...
          <input data-edit-start type="text" inputmode="text" value="${format12hFromMs(s.start)}">
        </label>
        <label class="field">
          <span>End time</span>
          <input data-edit-end type="text" inputmode="text" value="${format12hFromMs(s.end)}">
        </label>
      </div>
  ...
  exportCSV.addEventListener("click", ()=>{
    ...
    for(const s of filtered){
      const breaksField = (s.breaks||[]).map(b => `${b.type}|${Number(b.minutes)||0}`).join("; ");
      rows.push([
